...
const default_categories_1 = require("../src/common/default-categories");
...
async function main() {
    ...
    await prisma.plan.upsert({
        ...
        update: {},
        create: { id: 'free', name: 'Plano Gratuito', price: 0, description: 'Ate 3 contas, 20 transacoes/mes, categorias basicas, 1 usuario' },
    });
    await prisma.plan.upsert({
        where: { name: 'Plano Premium' },
        update: {},
        create: { id: 'premium', name: 'Plano Premium', price: 19.9, description: 'Contas e transacoes ilimitadas, relatorios PDF, insights, ate 3 usuarios, suporte prioritario' },
    });
    await prisma.plan.upsert({
        ...
        if (existing) {
            ...
            await prisma.subscription.upsert({
                ...
                create: {
                    ...
                    planId: demo.plan,
                    status: 'active',
                    value: demo.plan === 'premium' ? 19.9 : demo.plan === 'pro' ? 49.9 : 0,
                    nextDueDate,
                },
            });
            continue;
