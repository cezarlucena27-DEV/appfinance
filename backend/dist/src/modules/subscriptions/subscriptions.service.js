...
let SubscriptionsService = class SubscriptionsService {
    ...
    getPixKey() {
        try {
            const raw = (0, fs_1.readFileSync)(this.getPixConfigPath(), 'utf-8');
    ...
    buildPixPayload(pixKey, amount, txid) {
        const merchantName = 'FINANCEAPP';
        const merchantCity = 'SAO PAULO';
        const amountStr = amount.toFixed(2);
        let payload = this.emvField('00', '01') +
            this.emvField('01', '12') +
    ...
    async getPix(userId, planId) {
        if (!planId || planId === 'free') {
            throw new common_1.BadRequestException('Plano invalido para geracao de PIX');
        }
        ...
        if (!pixKey) {
            throw new common_1.BadRequestException('Chave PIX nao configurada pelo administrador');
        }
    ...
    computeNextDueDate(billingDay, from = new Date()) {
    ...
    async checkout(userId, plan, billingDay) {
        const membership = await this.prisma.workspaceMember.findFirst({
            where: { userId },
            include: { workspace: true },
        });
        if (!membership)
            throw new common_1.NotFoundException('User has no workspace');
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
            select: { createdAt: true },
        });
        const day = billingDay && billingDay >= 1 && billingDay <= 28
