...
export class AccountsService {
  ...
  async findAll(userId: string) {
    ...
    return all.map(({ shares, ...account }) => {
      ...
      if (!isGroup) return account;

      const userIds = new Set<string>();
      for (const a of groupAccounts) userIds.add(a.userId);
      for (const a of groupAccounts) for (const s of a.shares) userIds.add(s.userId);
  ...
  async getWorkspaceAccounts(masterUserId: string) {
    const membership = await this.prisma.workspaceMember.findFirst({
      where: { userId: masterUserId },
      include: { workspace: { include: { members: true } } },
    });
    if (!membership || membership.role !== 'master') {
      throw new ForbiddenException('Apenas o usuario master pode acessar as contas do workspace');
    }
  ...
  async getAccountTransactions(masterUserId: string, accountId: string) {
    const membership = await this.prisma.workspaceMember.findFirst({
      where: { userId: masterUserId },
      include: { workspace: { include: { members: true } } },
    });
    if (!membership || membership.role !== 'master') {
      throw new ForbiddenException('Apenas o usuario master pode acessar movimentacoes de contas');
    }
    ...
    if (!ownerIsMember) {
      throw new NotFoundException('Conta nao encontrada no workspace');
    }
  ...
  async exportWorkspaceExcel(masterUserId: string) {
    const membership = await this.prisma.workspaceMember.findFirst({
      where: { userId: masterUserId },
      include: { workspace: { include: { members: { include: { user: { select: { id: true, name: true, email: true } } } } } } },
    });
    if (!membership || membership.role !== 'master') {
      throw new ForbiddenException('Apenas o usuario master pode exportar os dados do workspace');
    }
  ...
  async create(userId: string, dto: CreateAccountDto) {
    ...
    if (accountCount >= limits[plan]) {
      throw new ForbiddenException('Limite de contas atingido para o seu plano');
    }
  ...
  async linkAccount(id: string, ownerUserId: string, targetUserId: string) {
    const account = await this.prisma.account.findUnique({ where: { id } });
    if (!account) throw new NotFoundException('Conta nao encontrada');

    const membership = await this.prisma.workspaceMember.findFirst({ where: { userId: ownerUserId } });
    if (!membership || membership.role !== 'master') {
      throw new ForbiddenException('Apenas o usuario master pode vincular contas');
    }
    if (account.userId !== ownerUserId) {
      throw new ForbiddenException('Somente o proprietario da conta pode vincula-la');
    }
    if (account.userId === targetUserId) {
      throw new ForbiddenException('A conta ja pertence a este usuario');
    }
    ...
    if (!targetMembership) {
      throw new ForbiddenException('O usuario deve ser membro do workspace');
    }
  ...
  async unlinkAccount(id: string, ownerUserId: string, targetUserId: string) {
    const account = await this.prisma.account.findUnique({ where: { id } });
    if (!account) throw new NotFoundException('Conta nao encontrada');

    const membership = await this.prisma.workspaceMember.findFirst({ where: { userId: ownerUserId } });
    if (!membership || membership.role !== 'master') {
      throw new ForbiddenException('Apenas o usuario master pode desvincular contas');
    }
    if (account.userId !== ownerUserId) {
      throw new ForbiddenException('Somente o proprietario da conta pode desvincula-la');
    }
  ...
  async remove(id: string, userId: string) {
    ...
    if (transactionCount > 0) {
      throw new ForbiddenException('Nao e possivel excluir conta com transacoes');
    }
